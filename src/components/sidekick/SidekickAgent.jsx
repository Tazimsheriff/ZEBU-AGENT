...
export const SidekickAgent = () => {
  ...
  // Floating Action Button (FAB) when closed or minimized
  if (!isOpen) {
    return (
      <button
        type="button"
        onClick={() => setIsOpen(true)}
        className={`fixed z-50 flex items-center gap-2 rounded-2xl bg-gradient-to-tr from-blue-700 to-cyan-500 text-white shadow-xl shadow-blue-600/30 hover:shadow-2xl hover:scale-105 active:scale-95 transition-all group ${
          isMobile
            ? 'bottom-20 right-4 px-3.5 py-2.5 text-xs font-bold'
            : 'bottom-6 right-6 w-14 h-14 justify-center'
        }`}
        title="Open mynt Sidekick AI Copilot"
      >
        <Sparkles className="w-5 h-5 group-hover:rotate-12 transition-transform shrink-0 text-cyan-200" />
        {isMobile && <span>Ask mynt AI</span>}
        <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-rose-500 rounded-full border-2 border-white animate-pulse" />
      </button>
    );
  }

  // Mobile Bottom Sheet vs Desktop Floating Panel
  return (
    <>
      {/* Backdrop for Mobile */}
      {isMobile && (
      ...
      <div
        className={
          isMobile
            ? 'fixed inset-x-0 bottom-0 top-14 z-50 max-w-lg mx-auto bg-white rounded-t-[28px] shadow-2xl flex flex-col overflow-hidden border-t border-slate-200 animate-in slide-in-from-bottom duration-300'
      ...
      >
        {/* Mobile Pull Handle */}
        {isMobile && (
          <div className="bg-[#05234e] flex justify-center pt-2 pb-1">
        ...
        <div className="px-4 py-1.5 bg-blue-50/80 border-b border-blue-100 flex items-center gap-2 text-[10px] text-blue-700 font-medium shrink-0">
          ...
          <span className="truncate">Connected to your live portfolio, mutual funds & IPO desk</span>
        ...
        >
          ...
          <p className="text-center text-[9px] text-slate-400 mt-1.5">
            Powered by Zebu AI • Gemini 2.5 Flash • Vercel Generative UI
          </p>
        </form>
      </div>
    </>
